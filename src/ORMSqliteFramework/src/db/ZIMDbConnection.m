...
#define DB_SECHEME_DOC [[NSUserDefaults standardUserDefaults] objectForKey:AresGlobalDomain]==nil?@"":[[NSUserDefaults standardUserDefaults] objectForKey:AresGlobalDomain]//app唯一标示（单用户登陆）
...
- (instancetype) initWithDictionary: (NSDictionary *)dictionary withMultithreadingSupport: (BOOL)multithreading {
	if ((self = [super init])) {
		...
		if ((readonly != nil) && [readonly boolValue]) {
            ...
            BOOL hasDoc = NO;
            if (!NEED_LOGIN) {//如果不要登陆，就不保存在文件夹中
                hasDoc = YES;
            }
            if ([DB_SECHEME_DOC isEqualToString:@""]) {//如果没有登陆，就不保存在文件夹中
                hasDoc = YES;
            }

            if (!hasDoc) {
                database  = [NSString stringWithFormat:@"%@_%@",NEED_LOGIN?DB_SECHEME_DOC:@"",database];
            }
	...
	@synchronized(self) {
		if (!_isConnected) {
			if (sqlite3_open([_dataSource UTF8String], &_database) != SQLITE_OK) {
                ...
                if (YT_IS_ENCRYPT_SCHEME) {
                    const char* key = [@"huhao" UTF8String];    //数据库文件加密
                ...
                if (![value integerValue]) {
                    if ([value isKindOfClass:[NSString class]]) {
                        if (YT_IS_ENCRYPT_DATA) {
                             value = [[FBDMManager singleton] performSelector:@selector(dd:) withObject:value];
                        }
                    }
                }
                NSString *DQL = [NSString stringWithFormat: @"set%@", [[[columnNames objectAtIndex: index] lowercaseString] capitalizedString]];
                if ([record respondsToSelector:NSSelectorFromString(DQL)]) {
                    [record performSelector:NSSelectorFromString(DQL) withObject:value];
                }
                /**
                 *  这里的kvo是个有问题的地方需要解决 add  by 胡皓 hh@yitong.com.cn
                 */
...
- (SEL) selectorForSettingColumnName: (NSString *)column {
    return NSSelectorFromString([NSString stringWithFormat: @"set%@", [[column lowercaseString] capitalizedString]]);
...
+ (NSNumber *) dataSource: (NSString *)dataSource execute: (NSArray *)array inMu:(BOOL)mul{
    ZIMDbConnection *connection = [[ZIMDbConnection alloc] initWithDataSource: dataSource withMultithreadingSupport: YES];
    __block NSNumber *result = nil;
    dispatch_queue_t queueOfquery = dispatch_queue_create("saveOfsave", NULL);
