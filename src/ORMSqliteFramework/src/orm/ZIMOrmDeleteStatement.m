...
#import "ZIMOrmDeleteStatement.h"
#import "DB_Header.h"
@implementation ZIMOrmDeleteStatement
...
- (instancetype) initWithModel: (Class)model {
	if ((self = [super init])) {
		...
		_model = model;
		_sql = [[ZIMSqlDeleteStatement alloc] init];
        [_sql table:[model table]];
	}
	return self;
}
/*
...
-(BOOL)deleteAll{
    ...
    if (connection) {
        NSNumber *result = [ZIMDbConnection dataSource: @"live" execute: [_sql statement]];
        if (result) {
            return YES;
        }
        return  NO;
...
-(BOOL)deleteByWhere:(NSDictionary *)where{
    ZIMDbConnection *connection = [[ZIMDbConnection alloc] initWithDataSource: [_model dataSource] withMultithreadingSupport: NO];
    ...
    if (connection) {
        ...
        NSNumber *result = [connection execute: [_sql statement]];
        [connection close];
        if (result) {
            return YES;
        }
